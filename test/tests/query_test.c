...
START_TEST (test_query_is_reachable)
{
    ...
    res=find_path_reachability(0,10,graph,vn);
    ck_assert_msg(res==true,"res should have been true but it was false");
    bitmap_clear_all(vn);
    res=find_path_reachability(4,12,graph,vn);
    ck_assert_msg(res==true,"res should have been true but it was false");
    bitmap_clear_all(vn);
    res=find_path_reachability(1,10,graph,vn);
    ck_assert_msg(res==false,"res should have been false but it was true");
    bitmap_clear_all(vn);
    res=find_path_reachability(19,2,graph,vn);
    ck_assert_msg(res==false,"res should have been false but it was true");
}
END_TEST
